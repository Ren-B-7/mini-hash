...
extern "C" {
	...
	typedef struct file_struct_impl file_struct;
	typedef struct file_struct_impl* file_t;

	typedef struct dir_struct_impl dir_struct;
	typedef struct dir_struct_impl* dir_t;
...
#define FS_PATH_SEPARATOR '/'
	...
	*/
	...
	/*  Build and return the canonical absolute path to the file using the path
	    ...
	    NOTE: Up to the caller to free the resulting memory
	    NOTE: Up to the caller to pass in an array large enough for the results
	   "res"
	*/
	char* fs_combine_filepath(const char* path, const char* filename);
	char*
	fs_combine_filepath_alt(const char* path, const char* filename, char* res);
	...
	*/
	...
	/*  Turn the permissions mode_t (int) into a printable format "drwxrwxrwx"
	    ...
	    NOTE: If res is memory already allocated to hold the string, it must be
	   at least 11 characters in length */
	...
	/*  Remove the directory pointed to by path; to remove all sub-directories
	   and files, use the `fs_rmdir_alt` function and set recursive to `true`.
	    Returns:
	        FS_NOT_VALID    - bad input
	        FS_NO_EXISTS    - directory already does not exist
	        FS_FAILURE      - error, likely read/write access or change during
	   operation FS_SUCCESS      - on success signifies that the directory no
	   longer exists
	*/
	int fs_rmdir(const char* path);
	int fs_rmdir_alt(const char* path, bool recursive);

	/*  List all the files and directories of the provided path. Items is a
	   passed by reference int that, on completion, will identify how many
	   records are returned. If error, items will be set to 0 Returns: NULL if
	   error or if path is not a directory
	*/
	 ...
	 *******************************************************************************/
	/*  Iniialize the dir_t object and pull information about the directory
	   pointed to by path Returns: dir_t   - Allocated memory with all members
	   set except for reading the directory into memory NULL    - If path is
	   invalid or is not a directory
	*/
	...
	/*  Return a list of strings that contain the full filepath to the item
	   (file, or directory);
	*/
	...
	/*  Initialize the file_t object and pull information about the file pointed
	   to by filepath Returns: NULL    - If filepath doesn't point to a file or
	   symlink file_t  - Allocated memory with all members set except for
	...
	*/
	...
	/*  Return access to the lines extracted, will be NULL if lines have not
	   been extracted from the file */
	char** f_lines(file_t f);

	/*  Return the full buffer contents of the file; originally will have
	   newlines (if text) but those will be set to \0 if lines are extracted.
	   This will allow for only needing to read the contents once. NOTE: Will be
	   of size f_filesize + 1 NOTE: If lines have been extracted, printing
	   buffer will only show the first line!
	*/
	...
	/*  Read the contents of file_t (if necessary) and modify buffer to place
	   NULLs at each \n\r\f characters; lines points to each location for the
	   line. This allows for not needing double the space for buffer and lines.
